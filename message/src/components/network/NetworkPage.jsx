...
const NetworkPage = () => {
        ...
        try {
            ...
            const [usersRes, connectionsRes] = await Promise.all([
                ...
                axios.get('http://localhost:5000/api/connections/status', {
                    headers: { Authorization: `Bearer ${token}` } // Status endpoint needs auth
                })
            ]);
    ...
    const handleConnect = async (userId) => {
        try {
            const token = localStorage.getItem('token');
            const res = await axios.post('http://localhost:5000/api/connect/request', {
                fromId: currentUserId,
                toId: userId
            }, {
                headers: { Authorization: `Bearer ${token}` } // Mock token handling might need adjustment
            });
    ...
    const handleAccept = async (userId) => {
        try {
            ...
            await axios.post('http://localhost:5000/api/connect/accept', {
            ...
            setConnections(prev => prev.map(c =>
                c.id === connection.id ? { ...c, status: 'accepted' } : c
            ));
        } catch (err) {
                    ...
                    {users.map(user => {
                        ...
                        return (
                            <div key={user.id} className="user-card" style={{
                                ...
                                <h3 style={{ margin: '0 0 5px', fontSize: '1.1rem' }}>{user.name}</h3>
